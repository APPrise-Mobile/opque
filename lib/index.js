...
export class OpQue {
  ...
  constructor(opts) {
    ...
    this.intervalId = setInterval(this._flush.bind(this), this.flushTime);
  ...
  queueOperation(operation, document) {
    validateOperation(operation);
    validateDocument(document, this.identifier);

    if(operation === CREATE) {
      this._queueCreate(document);
    } else if(operation === UPDATE) {
      this._queueUpdate(document);
    } else if(operation === DELETE) {
      this._queueDelete(document);
    }
  ...
  _queueCreate(doc) {
    ...
    this.queue[doc[this.identifier]] = operation;
  }
  ...
  _queueDelete(doc) {
    ...
    if(_.isObject(currentOperation) && currentOperation.operation === CREATE) {
      log.debug({currentOperation: currentOperation}, 'omitting create from queue because of delete');
