...
export class OpQue {
  ...
  queueOperation(operation, document, metaData) {
    ...
    let limitReached = this._isLimitReached();
    if(limitReached) {
      this.flush();
    } else {
  ...
  _isLimitReached() {
    if(_.isUndefined(this.opLimit)) {
      return false;
    }

    let opLimit = this.opLimit;
    if(_.isString(this.opLimit)) {
      opLimit = parseInt(this.opLimit, 10);
    }

    let currentOperationCount = Object.keys(this.queue).length;
    return currentOperationCount >= opLimit;
  }
